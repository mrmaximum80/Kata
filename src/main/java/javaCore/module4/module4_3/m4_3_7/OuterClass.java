package javaCore.module4.module4_3.m4_3_7;
...
class OuterClass {
    ...
    public static class UntrustworthyMailWorker implements MailService {
        ...
        @Override
        public Sendable processMail(Sendable mail) {
            Sendable xMail = mails[0].processMail(mail);
            for (int i = 0; i < mails.length - 1; i++) {
                xMail = mails[i + 1].processMail(xMail);
            }
            r = getRealMailService();
            return r.processMail(xMail);
        }
        ...
        public Sendable processMail(Sendable mail) {
            if (mail instanceof MailPackage) {
                if (((MailPackage) mail).getContent().getPrice() >= minPrice) {
                    ...
                    Package stones = new Package("stones instead of "
                            + ((MailPackage) mail).getContent().getContent(), 0);
                    mail = new MailPackage(((MailPackage) mail).getFrom(),
                            ((MailPackage) mail).getTo(), stones);
                }
            }
            return mail;
        }
